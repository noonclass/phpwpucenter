...
<?php if($oneself){ ?>
					...
					<?php
						if($currency=='cash'){$type='现金';$otype='积分';$sum_rewards=get_um_aff_sum_money($curauth->ID);$withdrawed = um_get_withdraw_sum($curauth->ID,'withdrawed');$withdrawing = um_get_withdraw_sum($curauth->ID,'withdrawing');$left = $sum_rewards-$withdrawed-$withdrawing;$link = um_get_user_url('affiliate',$curauth->ID).'&currency=credit';}else{$type='积分';$otype='现金';$sum_rewards=get_um_aff_sum_money($curauth->ID,'credit');$link = um_get_user_url('affiliate',$curauth->ID).'&currency=cash';}
