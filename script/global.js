...
/* 移动端导航VS幻灯片参数 */
$(function(){$('.bxslider').bxSlider({auto:true,captions:false,mode:'fade'});$('.slide-wrapper').on('touchstart','li',function(e){$(this).addClass('current').siblings('li').removeClass('current')});$('a.slide-menu').on('click',function(e){var wh=$('.wrapper').height();$('.slide-mask').css('height',wh).show();$('.slide-wrapper').css('height',wh).addClass('moved')});$('.slide-mask').on('click',function(){$('.slide-mask').hide();$('.slide-wrapper').removeClass('moved')});});
/* 返回顶部按钮 */
