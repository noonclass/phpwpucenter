...
function um_check_paybox(){
    ...
    $("#pb_submit").bind("click",function(){
        ...
        if(pb_account==''){alert('支付账号不能为空');$("#paybox input#pb_account").trigger("focus");return true;}
...
window.onload=function(){
  $('#bdshare_s').html('');
  setTimeout(function () { 
      $('.qrguide-area-img:first').toggleClass("background active");
      $('.qrguide-area-img:last').toggleClass("background active");
      $('.qrguide-area-img').fadeIn();
  }, 3000);
};

// Qrcode guide show
$('.qrguide-area-img').bind('click',function(){
    $('.qrguide-area-img:first').toggleClass("background active");
    $('.qrguide-area-img:last').toggleClass("background active");
    $('.qrguide-area-img').fadeIn();
});
...
$(".like-btn").click(function(){
	  ...
	  $.ajax({type: 'POST', xhrFields: {withCredentials: true}, dataType: 'json', url: moemob.ajax_url, data: 'action=like&pid=' + pid, cache: false, success: function(data){var num = _this.children("span").text();_this.children("span").text(Number(num)+1);_this.addClass("love-yes").attr("title","已喜欢");_this.children('i').attr('class','fa fa-heart');umSetCookie(cookie_name,1,3600,moemob.home_url);if(data.get===1) umAlert('参与文章互动获取'+data.credit+'积分');}});
});
...
function set_um_nonce(){
	$.ajax({
		type: 'POST', url: moemob.ajax_url, data: { 'action' : 'um_create_nonce' },
		success: function(response) {
			umSetCookie('um_check_nonce',$.trim(response),3600,moemob.home_url);
		},
		error: function(){
			set_um_nonce();
		}
...
function calculate() {
    ...
    $("#order_total").attr("value", Number($("#order_quantity").val() * $("#order_price").val()).toFixed(2));
};
...
$('#pay-submit').on('click',function create_order(){
	...
	var product_id = $('input#product_id').val(),
        ...
        payment_way = $('#order input[name="payment_way"]:checked').val(),
        payment_account = $('#order input#payment_account').val(),
    ...
    if(payment_account==''){alert('支付账号不能为空');$('#order input#payment_account').trigger("focus");$this.removeClass('disabled').text('立即付款');return false;}
...
$('#joinvip-submit').click(function create_vip_order(){
    ...
    var pb_way = $('#paybox input[name="pb_way"]:checked').val();
    var pb_account = $('#paybox input#pb_account').val();
	$.ajax({
			type: 'POST',
			dataType: 'json',
			async: true,
...
// Refresh captcha
  $('img.captcha_img').bind('click',function(){
  	var captcha = moemob.captcha + Math.random();
...
$(document).ready(function(){
	...
	if(umGetQueryString('aff')) umSetCookie('um_aff',umGetQueryString('aff'),86400,moemob.home_url);

	// action update traffic
	if(!(typeof(moemob.pid) == "undefined")) update_um_traffic(moemob.pid);
	...
	$("a#upload-cover").click(function(G){
		...
		tb_show("",moemob.admin_url+"media-upload.php?type=image&TB_iframe=true");
		window.send_to_editor=function(D){
			imgurl=$("img",D).attr("src");
			B.attr('src',moemob.timthumb+imgurl+"&w=240&h=64&zc=1&q=100");C.attr('src',moemob.timthumb+imgurl+"&w=1000&h=265&zc=1&q=100");
			var F = moemob.timthumb+imgurl+"&w=1000&h=265&zc=1&q=100";
